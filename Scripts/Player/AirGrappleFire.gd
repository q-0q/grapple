...
func on_enter():
		...
		Util.delayed_callback(self._on_timeout, 0.75)
...
func compute_holder_rotation_deg() -> float:
	return rad_to_deg(Vector2.DOWN.angle_to( FiringPoint.global_position - get_global_mouse_position()))
